...
- Mocking: Em testes futuros que envolvam APIs externas (como a do Google Gemini ou Wikipedia REST API), as requisições devem ser "mockadas" usando `unittest.mock.patch` para evitar bater em cotas de API reais e garantir que os testes rodem rapidamente de forma independente.
